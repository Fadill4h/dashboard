...
// Gambar ikon langsung ke OffscreenCanvas / canvas biasa (iOS-safe, no Blob URL)
function makeCanvasIcon(drawFn, size) {
...
// WiFi Router icon — Router ISP & AP Hotspot
const iconWifiRouter = makeCanvasIcon((ctx, s) => {
    const cx = s / 2;
    // Body
    ctx.beginPath();
    ctx.roundRect(s*0.1, s*0.52, s*0.8, s*0.26, s*0.08);
    ctx.fillStyle = 'rgba(200,184,255,0.92)';
    ctx.fill();
    // LED dots
    [[0.26,0.66,'#22ee88'],[0.42,0.66,'#22ddff'],[0.58,0.66,'#ff8844']].forEach(([x,y,c]) => {
        ctx.beginPath(); ctx.arc(x*s, y*s, s*0.048, 0, Math.PI*2);
        ctx.fillStyle = c; ctx.fill();
    });
    // Antena kiri
    ctx.beginPath(); ctx.moveTo(s*0.24, s*0.52); ctx.lineTo(s*0.14, s*0.24);
    ctx.strokeStyle = 'rgba(200,184,255,0.85)'; ctx.lineWidth = s*0.05; ctx.lineCap = 'round'; ctx.stroke();
    ctx.beginPath(); ctx.arc(s*0.14, s*0.22, s*0.048, 0, Math.PI*2);
    ctx.fillStyle = '#a855f7'; ctx.fill();
    // Antena kanan
    ctx.beginPath(); ctx.moveTo(s*0.76, s*0.52); ctx.lineTo(s*0.86, s*0.24);
    ctx.strokeStyle = 'rgba(200,184,255,0.85)'; ctx.lineWidth = s*0.05; ctx.lineCap = 'round'; ctx.stroke();
    ctx.beginPath(); ctx.arc(s*0.86, s*0.22, s*0.048, 0, Math.PI*2);
    ctx.fillStyle = '#a855f7'; ctx.fill();
    // Sinyal WiFi
    ctx.strokeStyle = '#a855f7'; ctx.lineWidth = s*0.042; ctx.lineCap = 'round';
    ctx.beginPath(); ctx.arc(cx, s*0.48, s*0.1, Math.PI, 0); ctx.stroke();
    ctx.globalAlpha = 0.6;
    ctx.beginPath(); ctx.arc(cx, s*0.48, s*0.18, Math.PI, 0); ctx.stroke();
    ctx.globalAlpha = 1;
    ctx.beginPath(); ctx.arc(cx, s*0.48, s*0.04, 0, Math.PI*2);
    ctx.fillStyle = '#a855f7'; ctx.fill();
}, 48);

// Switch/Hub icon — Mikrotik
const iconRB941 = makeCanvasIcon((ctx, s) => {
    // Body
    ctx.beginPath();
    ctx.roundRect(s*0.04, s*0.30, s*0.92, s*0.38, s*0.07);
    ...
    // Aksen kiri
    ctx.beginPath();
    ctx.roundRect(s*0.04, s*0.30, s*0.14, s*0.38, s*0.07);
    ctx.fillStyle = 'rgba(99,102,241,0.88)'; ctx.fill();
    // 4 port
    [[0.23],[0.37],[0.51],[0.65]].forEach(([x]) => {
        ctx.beginPath(); ctx.roundRect(x*s, s*0.40, s*0.09, s*0.18, s*0.02);
        ctx.fillStyle = '#071020'; ctx.strokeStyle = '#334488'; ctx.lineWidth = s*0.018;
        ctx.fill(); ctx.stroke();
    });
    // Uplink port
    ctx.beginPath(); ctx.roundRect(s*0.76, s*0.40, s*0.11, s*0.18, s*0.02);
    ctx.fillStyle = '#071020'; ctx.strokeStyle = '#f59e0b'; ctx.lineWidth = s*0.022;
    ctx.fill(); ctx.stroke();
    // LED row
    [[0.27,'#22dd66'],[0.38,'#22dd66'],[0.50,'#22aaff'],[0.62,'#22aaff'],[0.80,'#ffaa22']].forEach(([x,c]) => {
        ctx.beginPath(); ctx.arc(x*s, s*0.35, s*0.03, 0, Math.PI*2);
        ctx.fillStyle = c; ctx.fill();
    });
    // Kabel stub
    [0.27,0.38,0.50,0.62].forEach(x => {
        ctx.beginPath(); ctx.moveTo(x*s, s*0.58); ctx.lineTo(x*s, s*0.68);
        ctx.strokeStyle = '#445566'; ctx.lineWidth = s*0.028; ctx.lineCap = 'round'; ctx.stroke();
    });
}, 48);
...
class TopologyViz {
    ...
    drawNode(node, time) {
        ...
        // Icon: custom canvas icon or emoji
        if (node.customImg) {
            const imgSize = r * 1.5;
            ctx.save();
            ctx.beginPath();
